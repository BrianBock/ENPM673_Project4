...
if __name__ == '__main__':
    ...
    videopath="../media/IMG_0317.MOV"
    video = cv2.VideoCapture(videopath)
    frame_num=0
    # move the video to the start frame and adjust the counter
    video.set(1,frame_num)
    ret, color_frame = video.read() # ret is false if the video cannot be read
    if ret:
        if newROI:
            cv2.imshow('Frame',color_frame)
            (x,y,w,h) = cv2.selectROI("Frame", color_frame, fromCenter=False,showCrosshair=False)
    
        else:
            x,y,w,h=ROIs[dataset]

        color_template = color_frame[y:y+h,x:x+w]
        rect=(x,y,w,h)
